...
public class Oauth2ClientIntegrationTest {
    ...
    private final static Pair<String, String> AUTH_SERVER_AUTH_URI_PROP = Pair.of("spring.security.oauth2.client.provider" + ".baeldung.authorization-uri",
        "http://localhost:{PORT}/auth/realms/baeldung/protocol/openid-connect/auth");
    private final static Pair<String, String> AUTH_SERVER_TOKEN_PROP = Pair.of("spring.security.oauth2.client.provider" + ".baeldung.token-uri",
        "http://localhost:{PORT}/auth/realms/baeldung/protocol/openid-connect/token");
    private final static Pair<String, String> AUTH_SERVER_USERINFO_PROP = Pair.of("spring.security.oauth2.client.provider" + ".baeldung.user-info-uri",
    ...
    private final String BAELDUNG_LOGIN_URI = "/oauth2/authorization/baeldung";

    @Value("${spring.security.oauth2.client.provider.baeldung.authorization-uri}")
    private String authServerAuthorizationURL;

    @Value("${spring.security.oauth2.client.provider.baeldung.token-uri}")
    private String configuredTokenUri;

    @Value("${spring.security.oauth2.client.provider.baeldung.user-info-uri}")
    ...
    public void givenAuthServerAndResourceServer_whenPerformClientLoginProcess_thenProcessExecutesOk() throws Exception {
        ...
        result = this.webTestClient.get()
            ...
            .isFound()
            .expectHeader()
            .value(HttpHeaders.LOCATION, endsWith("lsso-client/"))
            .returnResult(Void.class);
