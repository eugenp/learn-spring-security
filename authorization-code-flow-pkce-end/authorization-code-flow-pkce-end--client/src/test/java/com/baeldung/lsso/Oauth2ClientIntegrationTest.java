...
public class Oauth2ClientIntegrationTest {
    ...
    public void givenAuthServerAndResourceServer_whenPerformClientLoginProcess_thenProcessExecutesOk() throws Exception {
        ...
        String authorizationURL = result.getResponseHeaders()
            .getFirst(HttpHeaders.LOCATION);
        Map<String, String> authorizationUrlParams = Arrays.stream(authorizationURL.split("&"))
        ...
        assertThat(authorizationUrlParams).containsEntry(PkceParameterNames.CODE_CHALLENGE_METHOD, "S256");
        assertThat(authorizationUrlParams).containsKey(PkceParameterNames.CODE_CHALLENGE);
        String codeChallenge = authorizationUrlParams.get(PkceParameterNames.CODE_CHALLENGE);
        assertThat(codeChallenge).matches("([a-zA-Z0-9\\-\\.\\_\\~]){43}");
        ...
        assertThat(mappedBody).containsKey(PkceParameterNames.CODE_VERIFIER);
        final String codeVerifier = mappedBody.get(PkceParameterNames.CODE_VERIFIER);
        assertThat(codeVerifier).matches("([a-zA-Z0-9\\-\\.\\_\\~]){128}");
        final String actualCodeChallenge = getCodeChallenge(codeVerifier);
        assertThat(actualCodeChallenge).isEqualTo(codeChallenge);
    ...
    private static String getCodeChallenge(String codeVerifier) throws NoSuchAlgorithmException {
