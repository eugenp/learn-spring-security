...
public class PossessionController {
    ...
    public Possession findOne(@PathVariable("id") final String id) {
    ...
    public ModelAndView create(@Valid Possession possession, Authentication authentication) {
        Possession created = new Possession();
        created.setName(possession.getName());
        created.setOwner(userRepository.findByEmail(authentication.getName()));
        created = possessionRepository.save(created);
        System.out.println(created);
        // permissionService.addPermissionForAuthority(possession, BasePermission.ADMINISTRATION, "ADMIN");
        permissionService.addPermissionForUser(created, BasePermission.ADMINISTRATION, authentication.getName());
        return new ModelAndView("redirect:/user?message=Possession created with id " + created.getId());
    }
