...
public class LssoClientUILiveTest {
    ...
    public void givenServerStarts_thenEnsurePageLoads() {
        ...
        standardWait.until(ExpectedConditions.visibilityOfElementLocated(By.className("projects-container")));
        WebElement button = chromeDriver.findElement(By.className("projects-container")).findElement(By.tagName("button"));
    ...
    public void givenWritePrivileges_thenWeCanCreateProjects() {
        ...
        WebElement createProjectElement = chromeDriver.findElement(By.className("create-project"));
        WebElement createProjectInput = createProjectElement.findElement(By.tagName("input"));
        createProjectInput.sendKeys("New project");

        WebElement createProjectButton = createProjectElement.findElement(By.tagName("button"));
    ...
    @Test
    public void givenProjectReadPrivileges_thenWeCanViewTasks() {
        login(Privileges.READ_AND_WRITE);

        // Count original number of projects
        standardWait.until(ExpectedConditions.visibilityOfElementLocated(By.className("projects")));
        List<WebElement> projectItems = chromeDriver.findElements(By.className("project-item"));
        assertThat(projectItems).isNotEmpty();
        WebElement firstProjectItem = projectItems.get(0);

        WebElement createProjectButton = firstProjectItem.findElement(By.tagName("button"));
        createProjectButton.click();
        ...
        String task = tasks.get(0).findElement(By.className("task-name")).getText();
        assertThat(task).contains("Task Id");
    }

    @Test
    @Disabled("Not determining privileges via UI")
